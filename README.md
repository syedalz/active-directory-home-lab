...
![OU structure in Active Directory Users and Computers](docs/screenshots/ou-tree.png)
*The Acme OU hierarchy with department OUs and populated users.*

### Users and groups
- Created user accounts across departments and department security groups.
- Automated bulk user provisioning with a **PowerShell script** (`New-ADUser` in a loop) — the real-world approach to account creation at scale. See [`scripts/bulk-create-users.ps1`](scripts/bulk-create-users.ps1) (the temporary password is prompted at runtime, never committed to source control).

![Bulk user creation via PowerShell](docs/screenshots/powershell-users.png)
*Users provisioned in bulk with PowerShell, landing in their department OUs.*
...
![GPOs linked to OUs in Group Policy Management](docs/screenshots/gpo-management.png)
*GPOs created and linked to their target OUs.*
...
- **PowerShell** — automated user provisioning with error handling
...
/scripts
  └── bulk-create-users.ps1  PowerShell user provisioning
